...
[EN]
Large Language Models (LLMs) are probabilistic token predictors. When applied to enterprise-grade monoliths, they often generate code that relies on Implicit States (e.g., ThreadLocal, System.currentTimeMillis, or global caches). Mathematical proofs indicate that under weak memory models, the presence of these Implicit States leads to Non-confluence. This breaks the bisimulation relationship between intensional and extensional semantics, causing catastrophic, silent failures in asynchronous production environments. 

[ZH]
大型語言模型（LLMs）本質上是基於機率的 Token 預測器。當應用於企業級單體系統時，AI 極易生成依賴「隱性狀態（Implicit States）」的程式碼（如全域變數、執行緒局部狀態或環境熵）。嚴格的數學證明指出：在弱記憶體模型（Weak Memory Model）與異構併發調度下，隱性狀態的存在將導致系統推導喪失匯合性（Non-confluence）。這會徹底破壞內涵語意與外延語意間的互模擬（Bisimulation）關係，進而在生產環境中引發靜默崩潰。
...
2. Projection (Extensionalization): Executes the Extensionalization Operator to calculate how the detected Implicit States must be elevated into explicit parameters.
...
2. 投影（外延化）： 執行外延化投影算子，計算如何將被偵測到的隱性狀態強行「提升」為顯式的介面參數。
...
[EN]
To enforce extensional semantics, add the following workflow to .github/workflows/aica-gatekeeper.yml. You MUST explicitly declare the path to your policy directory.

[ZH]
若要於專案中強制執行外延語意，請將以下設定檔加入 .github/workflows/aica-gatekeeper.yml。使用者必須顯式宣告政策目錄的路徑，以解除環境依賴。
    ...
    steps:
      ...
      - name: Execute AICA Neuro-Symbolic Gatekeeper
        ...
        with:
          ...
          rules-path: '.github/aica-rules'

```
## Defining Boundaries / 宣告邊界契約
[EN]
AICA relies on static JSON policies (Static Interface Contracts) stored in the explicitly declared rules directory (e.g., .github/aica-rules/). You can customize these policies to define your enterprise's Zero Trust boundaries. Example of banning implicit time coupling:
[ZH]
AICA 仰賴存放於顯式宣告目錄（如 .github/aica-rules/）下的靜態 JSON 政策（靜態介面契約）。你可以自定義這些政策，確立企業內部的零信任邊界。以下為阻斷隱性時間耦合的範例：
```json
{
  ...
  "illegal_invocations": [
    {
      "pattern": "Date.now",
      "reason": "Implicit temporal dependency. Inject a TimeProvider interface explicitly."
    }
  ]
}
...
[ZH]
本框架的目的不是提升 AI 生成品質，而是從結構上剝奪 AI 生成的自由度。如果你的團隊追求不顧拓樸架構的極致開發速度，本工具將嚴重拖慢你的部署節奏。我們只為追求絕對確定性（Absolute Determinism）的工程師服務。
